...
bool PConfig::LoadOptions(const char *const nConfigTemplate[][2], const char *nConfigFile, int nDepth)
{
    ...
    if(!ConfigFile)
    {
        Console->Print("%s Cant open file \"%s\"", Console->ColorText(RED, BLACK, "[Error]"),
            nConfigFile);
        return false;
    }

    if(nDepth)
        Console->Print("%s Including file %s (nesting level %d)",
            Console->ColorText(GREEN, BLACK, "[Info]"), nConfigFile, nDepth);
    else
        Console->Print("%s Loading configuration file %s",
            Console->ColorText(GREEN, BLACK, "[Info]"), nConfigFile);
    ...
    while(!feof(ConfigFile) && fgets(line, 255, ConfigFile))
    {
        ...
        else if(mIncludeRegEx->Search(line))
        {
            ...
            {
                Console->Print("%s Max nesting level (%d) reached. Can't include configuration file %s",
                    Console->ColorText(RED, BLACK, "[Error]"), CONFIG_MAXDEPTH,mIncludeRegEx->Match(1));
                NoError = false;
            }
        }
        else
        ...
        if(!Opt.empty() && !Val.empty())
        {
            ...
            if (Found)
            {
                if (GetOption(Opt) != "")
                {
                    Console->Print("%s trying to set option %s more than once (old value '%s' - new value '%s' ignored)",
                        Console->ColorText(YELLOW, BLACK, "[Warning]"), Opt.c_str(), GetOption(Opt).c_str(), Val.c_str());
                }
                else
                {
            ...
            {
                Console->Print("%s option %s unknow and ignored (value %s)",
                    Console->ColorText(YELLOW, BLACK, "[Warning]"), Opt.c_str(), Val.c_str());
            }
        }
    }
    ...
    if(!nDepth) // Check only if we're at top level (ie. not in an included file)
    {
        ...
        while(nConfigTemplate[i][0][0] != '\0')
        {
            if (GetOption(nConfigTemplate[i][0]) == "")
            {
                ...
                {
                    Console->Print("%s required option %s missing from config file",
                        Console->ColorText(RED, BLACK, "[Error]"), nConfigTemplate[i][0]);
                    NoError = false;
                }
                else
                {
                    Console->Print("%s using default value %s for option %s",
                        Console->ColorText(GREEN, BLACK, "[Info]"), nConfigTemplate[i][1],
                        nConfigTemplate[i][0]);
    ...
    if (NoError)
    {
        if(nDepth)
            ...
            Console->Print("%s Configuration file loaded",
                Console->ColorText(GREEN, BLACK, "[Success]") );
    }
    else
